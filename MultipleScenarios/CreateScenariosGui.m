...
% Last Modified by GUIDE v2.5 18-Oct-2017 09:09:21
...
% Choose default command line output for CreateScenariosGui
    ...
    % uiwait(handles.CreateScenariosGui);
    
    handles.CreateScenariosGui.Name=['Create Scenarios - No file loaded'];
            B=varargin{1};

    if ~isempty(getDefaultBuildingPropertiesWorkspace)
        handles.B=B;
        temp=getDefaultBuildingPropertiesWorkspace;
%         handles.B=B;
        handles.B.AmbientTemperature=temp.AmbientTemperature;
        handles.B.WindSpeed=temp.WindSpeed;
        handles.B.WindDirection=temp.WindDirection;
        handles.B.Temp=temp.Temp;
        handles.B.ZoneName=temp.ZoneName;
        handles.B.v=temp.v;
        handles.B.nZones=temp.nZones;
        handles.B.Zones=temp.Zones;
        handles.B.nS=temp.nS;
        handles.B.Sensors=temp.Sensors;
        handles.B.nLevel=temp.nLevel;
        handles.B.nPaths=temp.nPaths;
        handles.B.Openings=temp.Openings;
        B=handles.B;
    else
        handles.B=B;
    end
    ...
    handles.P=P;
    handles.SourceZones = P.SourceZones;
...
%     handles.MultipleScenariosRbutton.set=1;
    % Contaminant
    set(handles.SourceGenerationRate,'String',P.SourceGenerationRate);
    set(handles.StartTime,'String',P.StartTime);
    ...
    set(handles.StartTimePrc,'String',P.SourcePrc(2));
    set(handles.StartTimeSamples,'String',P.SourceSamples(2));
    set(handles.SourceDurationPrc,'String',P.SourcePrc(3));
    set(handles.SourceDurationSamples,'String',P.SourceSamples(3));
    ...
    switch P.ScenarioSelection
        case 'single'
            handles.SingleScenarioRbutton.Value=1;
            handles.MultipleScenariosRbutton.Value=0;
                        %%% Disable Input %%%
            %Contaminant Source
            handles.SourceGenerationRatePrc.Enable='off';
            handles.SourceGenerationRateSamples.Enable='off';
            handles.StartTimePrc.Enable='off';
            handles.StartTimeSamples.Enable='off';
            handles.SourceDurationPrc.Enable='off';
            handles.SourceDurationSamples.Enable='off';
            handles.SourcesMax.Max=1;
            handles.SourcesMax.Value=P.SourcesMax;

            % Wind Direction
            handles.WindDirectionPrc.Enable='off';
            handles.WindDirectionSamples.Enable='off';

            % Wind Speed
            handles.WindSpeedPrc.Enable='off';
            handles.WindSpeedSamples.Enable='off';

            % Ambient Temperature
            handles.AmbientTemperaturePrc.Enable='off';
            handles.AmbientTemperatureSamples.Enable='off';

            % Path Openings
            % set(handles.PathOpenings,'String',P.PathOpenings);
            handles.PathOpeningsPrc.Enable='off';
            handles.PathOpeningsSamples.Enable='off';
            
            %Disable Method sellection
            handles.Methodgrid.Enable='off';
            handles.Methodrandom.Enable='off';
            handles.RandomEdit.Enable='off';
            
        case 'multiple'
            handles.SingleScenarioRbutton.Value=0;
            handles.MultipleScenariosRbutton.Value=1;
                        %%% Disable Input %%%
            %Contaminant Source
            handles.SourceGenerationRatePrc.Enable='on';
            handles.SourceGenerationRateSamples.Enable='on';
            handles.StartTimePrc.Enable='on';
            handles.StartTimeSamples.Enable='on';
            handles.SourceDurationPrc.Enable='on';
            handles.SourceDurationSamples.Enable='on';
            handles.SourcesMax.Max=B.nZones;
            handles.SourcesMax.Value=P.SourcesMax;

            % Wind Direction
            handles.WindDirectionPrc.Enable='on';
            handles.WindDirectionSamples.Enable='on';

            % Wind Speed
            handles.WindSpeedPrc.Enable='on';
            handles.WindSpeedSamples.Enable='on';

            % Ambient Temperature
            handles.AmbientTemperaturePrc.Enable='on';
            handles.AmbientTemperatureSamples.Enable='on';

            % Path Openings
            % set(handles.PathOpenings,'String',P.PathOpenings);
            handles.PathOpeningsPrc.Enable='on';
            handles.PathOpeningsSamples.Enable='on';
            
            %Enable Method sellection
            handles.Methodgrid.Enable='on';
            handles.Methodrandom.Enable='on';
            if Methodrandom==1
                handles.RandomEdit.String=P.RandomSamples;
                handles.RandomEdit.Enable='on';
            end
            
    end
   
    %Dissable run button until scenarios are created
    handles.run.Enable='off';
    handles.run.BackgroundColor=[0.94 0.94 0.94];
...
if get(handles.Methodgrid, 'value')
    set(handles.Methodrandom, 'value', 0);
    set(handles.RandomEdit, 'enable', 'off');
    handles.P.Method = 'grid';
    
    if strcmp(handles.P.ScenarioSelection,'multiple')
        set(handles.SourceGenerationRateSamples, 'enable', 'on');
        set(handles.StartTimeSamples, 'enable', 'on');
        set(handles.SourceDurationSamples, 'enable', 'on');
        set(handles.WindDirectionSamples, 'enable', 'on');
        set(handles.WindSpeedSamples, 'enable', 'on');
        set(handles.AmbientTemperatureSamples, 'enable', 'on');
        set(handles.PathOpeningsSamples, 'enable', 'on');
    end
else
    set(handles.Methodrandom, 'value', 1);
    set(handles.RandomEdit, 'enable', 'on');
    handles.P.Method = 'random';
    
    if strcmp(handles.P.ScenarioSelection,'multible')
        set(handles.SourceGenerationRateSamples, 'enable', 'off');
        set(handles.StartTimeSamples, 'enable', 'off');
        set(handles.SourceDurationSamples, 'enable', 'off');
        set(handles.WindDirectionSamples, 'enable', 'off');
        set(handles.WindSpeedSamples, 'enable', 'off');
        set(handles.AmbientTemperatureSamples, 'enable', 'off');
        set(handles.PathOpeningsSamples, 'enable', 'off'); 
    end
end
% Update handles structure
guidata(hObject, handles);
...
if get(handles.Methodrandom, 'value')
    set(handles.Methodgrid, 'value', 0);
    set(handles.RandomEdit, 'enable', 'on');
    handles.P.Method = 'random';
    set(handles.SourceGenerationRateSamples, 'enable', 'off');
    set(handles.StartTimeSamples, 'enable', 'off');
    ...
    set(handles.Methodgrid, 'value', 1);
    set(handles.RandomEdit, 'enable', 'off');
    handles.P.Method = 'grid';
    set(handles.SourceGenerationRateSamples, 'enable', 'on');
    set(handles.StartTimeSamples, 'enable', 'on');
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    B=handles.B;
    B.WindDirection = P.WindDirection;
    B.WindSpeed = P.WindSpeed;
    B.AmbientTemperature = P.AmbientTemperature;
    B.v = P.ZonesVolume;
    B.Temp = P.ZoneTemperature;
    B.Openings = P.PathOpenings;
    
    [B.A Qext Flows]=computeAmatrix(B,P.WindDirection,...
                                    P.WindSpeed,...
    ...
    if ~isempty((file0))
        ...
        try
            if exist([pwd,'\RESULTS\',file0(1:end-2),'.w'],'file')==2
                delete([pwd,'\RESULTS\',file0(1:end-2),'.w']);
            end
        catch
        end
         handles.msg=['Created Scenarios File: "',file0,'"'];
         uiwait(msgbox(handles.msg));

    handles.file0 =file0(1:end-2);
    
    handles.CreateScenariosGui.Name=['Create Scenarios - "' file0 '"'];
    
        %Enable Run button
            handles.run.Enable='on';
        handles.run.BackgroundColor=[0 1 0];
        guidata(hObject, handles);
...
%             handles.msg = '';
            ...
            if exitflag~=-1
                handles.msg = ['The simulation of "', handles.file0 ,'" is completed']; 
                uiwait(msgbox(handles.msg));
%              guidata(hObject, handles);
                ...
                % Update handles structure
                guidata(hObject, handles);
            % Extract updated data in workspace if Building Properties
            % Structure exists
            W = evalin('base','whos'); %or 'base'
            if ismember('DefaultBuildingProperties',[W(:).name])

                setDefaultBuildingPropertiesWorkspace(handles.B)
            end

            % Close Create Scenarios Gui and Resume
            uiresume
            else
                return
            end
...
function errorCode = CheckForError(handles)
    ...
    if ~length(StartTime) || StartTime<0 || (SimulationTime-StartTime-SourceDuration) <= 0 
    ...
    if  ~length(PathOpenings) || PathOpenings<0 || PathOpenings>1
        msgbox('         Wrong Path Openings', 'Error', 'modal')
        errorCode=1;
        return
    end
    ...
    StartTimePrc=(str2double(get(handles.StartTimePrc,'String')));
    if isnan(StartTimePrc) || StartTimePrc<0
        msgbox('Give variance of Start Time', 'Error', 'modal')
        errorCode=1;
        return
    end
    ...
    StartTimeSamples=str2num(char(get(handles.StartTimeSamples, 'String')));
    if ~length(StartTimeSamples) || StartTimeSamples<0
        msgbox('Give Samples of Start Time', 'Error', 'modal')
        errorCode=1;
        return
    end
    ...
    if  grid && random || ~grid && ~random
        msgbox('          Select Method', 'Error', 'modal')
        errorCode=1;
        return
    end
    
    if random 
        RandomSamples=str2num(char(get(handles.RandomEdit, 'String')));       
        if ~length(RandomSamples) || RandomSamples<0 || RandomSamples ~= int16(RandomSamples) 
            msgbox('Give Samples of Random', 'Error', 'modal')
            errorCode=1;
            return             
        end
    end
    ...
    if get(handles.SourcesMax,'Value') > length(handles.SourceZones)
    
        uiwait(msgbox('    The selected Zones must be greater than Source Max number',...
                      'Error', 'modal'))
        errorCode=1;
        return
    end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if isnumeric(file0)% true if a file is not sellected
        file0=[];
    else
        handles.CreateScenariosGui.Name=['Create Scenarios - "' file0 '"'];
    end
...
% --- Executes when user attempts to close CreateScenariosGui.
function CreateScenariosGui_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to CreateScenariosGui (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in SelectZone.
...
arguments.ZoneName = handles.B.ZoneName;
arguments.chooseZone(1:handles.B.nZones) = false;

for i= handles.SourceZones
    arguments.chooseZone(i) = true;  
end

[A B]= SelectZones(arguments);

if get(handles.SourcesMax,'Value') > length(find(B.chooseZone))
...
for i= 1:handles.B.nZones
    if B.chooseZone(i)
        handles.SourceZones = [handles.SourceZones, i]; 
    end
end
...
% --- Executes on button press in SensorData.
...
function RandomEdit_Callback(hObject, eventdata, handles)
% hObject    handle to RandomEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of RandomEdit as text
%        str2double(get(hObject,'String')) returns contents of RandomEdit as a double

handles.P.RandomSamples = str2double(get(handles.RandomEdit, 'String'));

guidata(hObject, handles);


% --- Executes during object creation, after setting all properties.
function RandomEdit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to RandomEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function StartTimePrc_Callback(hObject, eventdata, handles)
% hObject    handle to StartTimePrc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of StartTimePrc as text
%        str2double(get(hObject,'String')) returns contents of StartTimePrc as a double


% --- Executes during object creation, after setting all properties.
function StartTimePrc_CreateFcn(hObject, eventdata, handles)
% hObject    handle to StartTimePrc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function StartTimeSamples_Callback(hObject, eventdata, handles)
% hObject    handle to StartTimeSamples (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of StartTimeSamples as text
%        str2double(get(hObject,'String')) returns contents of StartTimeSamples as a double


% --- Executes during object creation, after setting all properties.
function StartTimeSamples_CreateFcn(hObject, eventdata, handles)
% hObject    handle to StartTimeSamples (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in SingleScenarioRbutton.
function SingleScenarioRbutton_Callback(hObject, eventdata, handles)
% hObject    handle to SingleScenarioRbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    if handles.SingleScenarioRbutton.Value
        P=DefaultParameters(handles,'single');
        handles.P=P;

        SetGuiParameters(handles.P,handles.B,handles);
        handles.MultipleScenariosRbutton.Value=0;
        ...
        P=DefaultParameters(handles,'multiple');
        handles.P=P;

        SetGuiParameters(handles.P,handles.B,handles);
        handles.MultipleScenariosRbutton.Value=1;
        
        %Anable and set Default method
%         handles.Methodgrid.Value=1;
%         handles.P.Method = 'grid';
        handles.Methodgrid.Enable='on';
        handles.Methodrandom.Enable='on';
...
% Update handles structure
    guidata(hObject, handles);
% Hint: get(hObject,'Value') returns toggle state of SingleScenarioRbutton


% --- Executes on button press in MultipleScenariosRbutton.
function MultipleScenariosRbutton_Callback(hObject, eventdata, handles)
% hObject    handle to MultipleScenariosRbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    if handles.MultipleScenariosRbutton.Value
        P=DefaultParameters(handles,'multiple');
        handles.P=P;
...
% Update handles structure
    guidata(hObject, handles);
% Hint: get(hObject,'Value') returns toggle state of MultipleScenariosRbutton
