...
%
% Modifications: Alexis Kyriacou, KIOS CoE, 2017.
%
%-- Added condition for two bars to finish in order to close
%-- Modified percentage text in title in order to show percentage for all
%-- bars
...
% Modified to close the figure after all inputs are finished
counter=0;
...
% Update progress figure title bar
%%%%%%%%%%% Modified title to post total bars percentage %%%%%%%%%%%%%%%%%
total_fractiondone=0;
total_runtime=0;
total_timeleft=0;
% if length(progdata) > 1
    for ndx = 1:length(progdata)
        total_fractiondone=total_fractiondone+(progdata(ndx).fractiondone)/length(progdata);
        if (progdata(ndx).fractiondone)~=1
            total_runtime=total_runtime+etime(progdata(ndx).clock, progdata(ndx).starttime);
        end
    end
    total_timeleft =total_runtime /total_fractiondone - total_runtime;
% else
%     
% end

if total_fractiondone > 0
    runtime =total_runtime;
    timeleft = total_timeleft;
    timeleftstr = sec2timestr(timeleft);
    titlebarstr = sprintf('%2d%%    %s remaining', ...
        floor(100*total_fractiondone), timeleftstr);
else
    titlebarstr = ' 0%';
end
...
% to display a question dialog box 
   selection = questdlg('Close Progress-Bar Figure and End the Process?',...
      'Close Request Function',...
      'Yes','No','Yes'); 
   switch selection, 
      case 'Yes',
