...
%     [s1 s2 s3] = size(handles.F.IsolationResidual);
    set(handles.Isolator, 'String', 1:handles.B.nZones);    
...
% handles    structure with handles and user data (see GUIDATA)
    if get(handles.DetectionThreshold, 'Value')||get(handles.DetectionResidual, 'Value')
        ...
        if get(handles.DetectionThreshold, 'Value')&~get(handles.DetectionResidual, 'Value')
            ...
            if length(zones)==1
                plot(handles.t(1:DT), handles.F.DetectionThreshold(:,zones),'--')
                title(sprintf('CDE at Zone %d.%s',zones,handles.B.ZoneName{zones}))
                xlabel('Time (h)')
                ylim([-0.5 max(handles.F.DetectionThreshold(:,zones))+0.5])
                legend('Threshold', 'Location', 'Best')                
            else
                for i=1:length(zones)
                    subplot(round(length(zones)/2),2,i)
                    plot(handles.t(1:DT), handles.F.DetectionThreshold(:,zones(i)),'--')
        ...
        if ~get(handles.DetectionThreshold, 'Value')&get(handles.DetectionResidual, 'Value')
            ...
            set(figure, 'Name', 'Detection Results');
            if length(zones)==1
                plot(handles.t(1:DT), handles.F.DetectionResidual(:,zones),'r')
                title(sprintf('CDE at Zone %d.%s',zones,handles.B.ZoneName{zones}))
                xlabel('Time (h)')
                legend('Residual', 'Location', 'Best')
                ylim([-0.5 max(handles.F.DetectionResidual(:,zones))+0.5])
            else
                for i=1:length(zones)
                    subplot(round(length(zones)/2),2,i)
                    plot(handles.t(1:DT), handles.F.DetectionResidual(:,zones(i)),'r')
                    title(sprintf('CDE at Zone %d.%s',zones(i),handles.B.ZoneName{zones(i)}))
                    xlabel('Time (h)')
        ...
        if get(handles.DetectionThreshold, 'Value')&get(handles.DetectionResidual, 'Value')
            ...
            if length(zones)==1
                ...
                title(sprintf('CDE at Zone %d.%s',zones,handles.B.ZoneName{zones}))
                xlabel('Time (h)')
                ylim([-0.5 max([handles.F.DetectionResidual(:,zones)', handles.F.DetectionThreshold(:,zones)'])+0.5])
                ...
                for i=1:length(zones)
                    ...
                    title(sprintf('CDE at Zone %d.%s',zones(i),handles.B.ZoneName{zones(i)}))
                    xlabel('Time (h)')
                    ylim([-0.5 max([handles.F.DetectionResidual(:,zones(i))', handles.F.DetectionThreshold(:,zones(i))'])+0.5])
                end
                legend('Residual', 'Threshold', 'Location', 'Best')
            end
    ...
    if get(handles.IsolationThresholdResidual, 'Value')||get(handles.SourceEstimation, 'Value')
        ...
        if get(handles.IsolationThresholdResidual, 'Value')&~get(handles.IsolationResidual, 'Value')&get(handles.IsolationThreshold, 'Value')
            ...
            if length(zones)==1
                ...
                plot(handles.t(DT:(DT-1+length(handles.F.IsolationThreshold(Isolator).Eyj(zones,:)))), handles.F.IsolationThreshold(Isolator).Eyj(zones,:),'--')
                title(sprintf('CIE%d at Zone %d.%s',Isolator ,zones,handles.B.ZoneName{zones}))
                xlabel('Time (h)')
                ylim([-0.5 max(handles.F.IsolationThreshold(Isolator).Eyj(zones,:))+0.5])
                legend('Threshold', 'Location', 'Best')                
            else
                for i=1:length(zones)
                    subplot(round(length(zones)/2),2,i)
                    plot(handles.t(DT:(DT-1+length(handles.F.IsolationThreshold(Isolator).Eyj(zones(i),:)))), handles.F.IsolationThreshold(Isolator).Eyj(zones(i),:),'--')
                    title(sprintf('CIE%d at Zone %d.%s',Isolator ,zones(i),handles.B.ZoneName{zones(i)}))
                    xlabel('Time (h)')
                    ylim([-0.5 max(handles.F.IsolationThreshold(Isolator).Eyj(zones(i),:))+0.5])
                end
                legend('Threshold', 'Location', 'Best')
            end
        ...
        if ~get(handles.IsolationThreshold, 'Value')&get(handles.IsolationResidual, 'Value')&get(handles.IsolationThresholdResidual, 'Value')
            ...
            set(figure, 'Name', 'Isolation Results');
            if length(zones)==1
                plot(handles.t(DT:(DT-1+length(handles.F.IsolationResidual(Isolator).Eyi(zones,:)))),  handles.F.IsolationResidual(Isolator).Eyi(zones,:),'r')
                title(sprintf('CIE%d at Zone %d.%s',Isolator ,zones,handles.B.ZoneName{zones}))
                xlabel('Time (h)')
                ylim([-0.5 max(handles.F.IsolationResidual(Isolator).Eyi(zones,:))+0.5])
                legend('Residual', 'Location', 'Best')                
            else
                for i=1:length(zones)
                    subplot(round(length(zones)/2),2,i)
                    plot(handles.t(DT:(DT-1+length(handles.F.IsolationResidual(Isolator).Eyi(zones(i),:)))), handles.F.IsolationResidual(Isolator).Eyi(zones(i),:),'r')
                    title(sprintf('CIE%d at Zone %d.%s',Isolator ,zones(i),handles.B.ZoneName{zones(i)}))
                    xlabel('Time (h)')
                    ylim([-0.5 max(handles.F.IsolationResidual(Isolator).Eyi(zones(i),:))+0.5])
                end
                legend('Residual', 'Location', 'Best')
            end
        ...
        if get(handles.IsolationThreshold, 'Value')&get(handles.IsolationResidual, 'Value')&get(handles.IsolationThresholdResidual, 'Value')
            ...
            if length(zones)==1
                hold on                
                plot(handles.t(DT:(DT-1+length(handles.F.IsolationResidual(Isolator).Eyi(zones,:)))), handles.F.IsolationResidual(zones,:,Isolator),'r')
                plot(handles.t(DT:(DT-1+length(handles.F.IsolationThreshold(Isolator).Eyj(zones,:)))), handles.F.IsolationThreshold(Isolator).Eyj(zones,:),'--')
                title(sprintf('CIE%d at Zone %d.%s',Isolator, zones,handles.B.ZoneName{zones}))
                xlabel('Time (h)')
                ylim([-0.5 max([handles.F.IsolationResidual(zones,:,Isolator), handles.F.IsolationThreshold(Isolator).Eyj(zones,:)])+0.5])
                legend('Residual', 'Threshold', 'Location', 'Best')                
            else
                for i=1:length(zones)
                    subplot(round(length(zones)/2),2,i)
                    ...
                    title(sprintf('CIE%d at Zone %d.%s',Isolator, zones(i),handles.B.ZoneName{zones(i)}))
                    xlabel('Time (h)')
                    ylim([-0.5 max([handles.F.IsolationResidual(Isolator).Eyi(zones(i),:), handles.F.IsolationThreshold(Isolator).Eyj(zones(i),:)])+0.5])
                end
                legend('Residual', 'Threshold', 'Location', 'Best')
            end
